...
resourcename="checklistgroup"  # lower case, only
resourcegroup="checklistgroup_rg"
...
# git clone https://github.com/apilogicserver/checklistgroup.git
# cd checklistgroup
...
az group create --name checklistgroup_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group checklistgroup_rg --sku S1 --is-linux
az container create --resource-group checklistgroup_rg --name checklistgroup --image apilogicserver/checklistgroup:latest --dns-name-label checklistgroup --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//checklistgroup.westus.azurecontainer.io:5656'
